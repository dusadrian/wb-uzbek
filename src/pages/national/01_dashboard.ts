...
import { regions } from "../../libraries/administrative";
...
export const nationalDashboard = {
    init: async () => {
        ...
        sessionStorage.setItem('topMenuPath', 'national');
        ...
        const region = (<HTMLSelectElement>document.getElementById('region'))
        for (const reg in regions) {
            const option = document.createElement('option');
            option.value = reg;
            option.innerText = reg + ' | ' + regions[reg][lang as 'en' | 'uz' | 'ru'];
            region.appendChild(option);
        }
        region.addEventListener('change', () => {
            regionCode = region.options[region.selectedIndex].value;
            (<HTMLSelectElement>document.getElementById('institutionType')).value = '';
            (<HTMLSelectElement>document.getElementById('institution')).selectedIndex = 0;
            (<HTMLSelectElement>document.getElementById('institution')).dispatchEvent(new Event('change'));
            ipcRenderer.send('getRegionalDashStats', {
                region: regionCode,
